...
#                  [0.388, 0.063, 0.900, -2.902, -1.202, 0]])
# 控制盒盒形示教位置
Pos0 = np.array([[0.437, 0.117, 0.900, -2.902, -1.202, 0],
                 [0.387, 0.117, 0.900, -2.902, -1.202, 0],
                 [0.437, 0.063, 0.900, -2.902, -1.202, 0],
                 [0.388, 0.063, 0.900, -2.902, -1.202, 0]])
PosOri = np.array([[0.326, 0.097, 0.889, -2.902, -1.202, 0]])
# 示教相机-螺杆位置
dx = -(131.09 - 96.78)
dy = -(429.68 - 340.23) +1
# 插入深度
...
for i in range(4):
    ...
    while 1:
        ...
        count = count + 1
        # 检测孔位并记录图像
        circlePos, ImgDraw = pd.getCirclePosEDWithRadiusEstimate(Img, 1, 50, 10, 10, dRuler, minRadius, maxRadius)
