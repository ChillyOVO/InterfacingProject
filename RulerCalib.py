...
for i in range(3):
    for j in range(3):
        if i == 1 and j == 1:
            ...
            dy = j * 0.01 - 0.01
            # 移动机械臂到位
            pd.getUR10EMove(IPRob, PortNumber, posX + dx, posY + dy, 0.7, 2.902, 1.202, 0)
            # 检测圆
            count = 0
            while 1:
                count = count + 1
                ...
                Height, Width = np.shape(ImgGray)
                # 检测圆
                circlePosNew, ImgDraw = pd.getCirclePosEDWithRadiusEstimate(Img, 1, 50, 10, 50, dRuler0, minRadius,
                                                                            maxRadius)
