...
while 1:
    ...
    CirclePoints = circlePos[:, 0:2]
    hull = cv2.convexHull(np.float32(CirclePoints))
    a = cv2.contourArea(hull)
    print(a)
    # print(np.array(hull[:,0]))
    # temp = np.zeros(ImgDraw.shape, np.uint8)
    # hull = np.round(hull)
    # ImgDraw = cv2.drawContours(ImgDraw, hull, -1, (0, 255, 0), 2)
    # 图示显示
    cv2.namedWindow("Test", 0)
    cv2.resizeWindow("Test", 1920, 1080)
    cv2.imshow('Test', ImgDraw)
...
for i in range(4):
    print('移动至目标孔', i + 1)
    # M3 检测高度
    pd.getUR10EMove(IPRob, 30003, 0.315, PosNow0[1] + Move[i, 1] / 1000, PosNow0[2] + Move[i, 0] / 1000, 1.760,
                    0.729, 1.760)
    count = 0
    countN = 0
    Img = pd.getSingleImageByBasler()
    ...
    while 1:
        ...
        # 检测是否满足条件，不满足条件则置空
        for j in range(num):
            if 1.95 > circlePos[j][2] * dRuler > 1.40:
                Dis = pd.getDis2D(circlePos[j][1], circlePos[j][0], Height / 2, Width / 2)
                if Dis < 600:
                    # print('检测成功')
                    print('第', countN * 10 + count, '次检测,检测成功')
                    # PosNow = pd.getUR10EPose(IPRob, PortNumber)
                else:
                    circlePos[j].fill(0)
            else:
        ...
        if len(circlePos[:, 0]) == 1:
            ...
            if count < 10:
                ...
                pz = p[0] * 0.01 - 0.005
                py = p[1] * 0.01 - 0.005
                pd.getUR10EMove(IPRob, 30003, 0.315, PosNow0[1] + Move[i, 1] / 1000 + py,
                                PosNow0[2] + Move[i, 0] / 1000 + pz, 1.760, 0.729, 1.760)
                Img = pd.getSingleImageByBasler()
                Img = cv2.undistort(Img, Intrinsic, Distortion)
    # 图示显示
    # cv2.namedWindow("Test", 0)
    ...
    pd.getUR10EMove(IPRob, 30003, 0.463, PosNow[1] + MoveM3Y / 1000, PosNow[2] + MoveM3Z / 1000, 1.760, 0.729, 1.760)
    sleep(10)
    pd.getUR10EMove(IPRob, 30003, 0.315, PosNow[1] + MoveM3Y / 1000, PosNow[2] + MoveM3Z / 1000, 1.760, 0.729, 1.760)
